---
title: 'Earth Nodes'
description: 'Run Earth Nodes to Decentralize the Bitcoin Social Network'
icon: 'earth-asia'
---
<img
...
Indexing satoshis is done using the Ordinals protocol, using the [Ordhook](https://github.com/earthstaking/ordhook) repo. To run this you will need to install [Rust](https://www.rust-lang.org/tools/install):
...
Lastly, we can start our Ordhook event listener to post real time Bitcoin inscriptions to your Earth Relay:
```
ordhook service start --predicate-path=relay-settings.json --config-path=Ordhook.toml
```
